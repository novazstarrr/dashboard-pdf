...
const useUsers = () => {
  ...
  const addUser = useCallback(async (userData) => {
    try {
      // Convert dob to ISO string before sending
      ...
      // Ensure that the response includes a valid dob
      const newUser = {
        ...response.data,
        dob: response.data.dob ? new Date(response.data.dob).toISOString() : "0001-01-01T00:00:00Z",
      };

      setUsers((prevUsers) => {
        const newUsers = [...prevUsers, newUser];
        const newTotalPages = Math.ceil(newUsers.length / ITEMS_PER_PAGE) || 1;
        setCurrentPage(newTotalPages); // Navigate to the last page
        return newUsers;
      });
  ...
  const editUser = useCallback(async (userId, userData) => {
    try {
      ...
      const response = await axios.put(`/api/users/${numericId}`, formattedData);

      console.log('PUT Response Data:', response.data); // {message: 'User updated successfully'}

      // Manually update the user in state without relying on response.data
      setUsers((prevUsers) => {
        const updatedUsers = prevUsers.map((user) =>
          user.id === numericId ? { ...user, ...formattedData } : user
        );
