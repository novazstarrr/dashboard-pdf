...
func (h *UserHandler) GetCurrentUser(w http.ResponseWriter, r *http.Request) {
    ...
    log.Printf("Raw userID from context: %v (type: %T)", userIDRaw, userIDRaw)
    ...
    case json.Number:
        n, err := v.Int64()
        if err != nil {
            log.Printf("Failed to convert json.Number to int64: %v", err)
            utils.RespondWithError(w, domain.NewAPIError(
                http.StatusBadRequest,
                domain.ErrCodeInvalidInput,
    ...
    log.Printf("Converted userID: %d", userID)

    if userID == 0 {
        log.Printf("User ID is 0")
        utils.RespondWithError(w, domain.NewAPIError(
            http.StatusBadRequest,
            domain.ErrCodeInvalidInput,
            "Invalid user ID: cannot be 0",
            nil,
        ))
        return
    }
